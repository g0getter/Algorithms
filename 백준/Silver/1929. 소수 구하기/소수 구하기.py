...
def find_prime_list_under_number_2(number):
    ...
    # 작거나 같은 정수 - 딱 떨어지는 경우에도 해야 함. 4->2면 2까지 검사해야함 -> range( , intRootNumber+1)
    int_floor_root_number = int(root_number)
...
def find_prime_list_under_number_optimzed(m, n):
    # 2부터 root number까지 돌며
    # 자기 자신은 그대로 두고, 배수들을 제거
...
result = find_prime_list_under_number_optimzed(M, N)
